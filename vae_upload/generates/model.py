...
class VAE(nn.Module):
    ...
    def likelihood(self, x):
        ...
        recon_loss, nll_sum = self.forward_decoder2(x,z)
        return  nll_sum
    
    def forward_decoder2(self,x,z):
        ...
        target = x[:, 1:] 
        log_probs = log_probs[:, :target.size(1),:]
        nll_loss = F.nll_loss(log_probs.transpose(1,2), target, reduction='none', ignore_index=self.pad)
        nll_sum = nll_loss.sum(dim=1)
        recon_loss = nll_loss.mean()
        return recon_loss, nll_sum
    ...
    def sample_ahc(self, n_batch, max_len=100):
        with torch.no_grad():
            nlls = torch.zeros(n_batch, device=self.device) 
            action_probs = torch.zeros((n_batch, max_len+1), requires_grad=True, device=self.device)
            action_log_probs = torch.zeros((n_batch, max_len+1), requires_grad=True, device=self.device)
            ...
            for i in range(1, max_len):
                ...
                for j in range(n_batch):
                    if not eos_mask[j]:
                        action_probs[j, i] = probs[j, w[j]]
                        action_log_probs[j, i] = log_probs[j, w[j]]
                        nlls[j] -= log_probs[j, w[j]]
            ...
            action_probs = action_probs[:, :max(end_pads)]
            action_log_probs = action_log_probs[:, :max(end_pads)]
        
        
            new_x = [x[i, :end_pads[i]] for i in range(x.size(0))]
            return [self.tensor2string(i_x) for i_x in new_x], nlls, action_probs, action_log_probs
